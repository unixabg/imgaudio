...
Two tuning systems, selected with --base:
  ...
  --base 60             Just intonation. Every degree is a small whole-number
                        ...
                        which is why base 60 was chosen for arithmetic in the
                        first place, and why Mesopotamian tuning was built
                        from cycles of fifths and fourths.

Note that base-60 tunings derived from fifths (babylonian, babylonian_pent)
land within ~6 cents of equal temperament. The audible difference comes from
scales using higher partials — `harmonic` reaches ~49 cents, close to a
quarter-tone.
...
RATIO_SCALES = {
    ...
    # Furthest from equal temperament; the 7th and 11th partials are the
    # notes that have no equal-tempered equivalent at all.
